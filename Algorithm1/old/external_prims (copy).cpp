...
{
	//const unsigned volume = (const unsigned) no_edges;
	
	typedef stxxl::PRIORITY_QUEUE_GENERATOR < edge, mst_cmp, 512 *1024 * 1024, 1024*1024*sizeof(edge)> pq_gen;	
	...
	*/
	...
	for(;itr!=g.edge_list.end() && itr->src == g.vmain[index-1].first.vertex_id ;itr++)
	{
		//STXXL_MSG(" (" <<(itr->src)<<", " <<(itr->dst)<<", "<<(itr->edge_wt)<<") ");
		pq_mst.push(*itr);
	}
